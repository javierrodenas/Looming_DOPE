...
def loadjson(path, objectsofinterest, img):
    ...
    for i_line in range(len(data['objects'])):
        ...
        """
        if not objectsofinterest is None and not objectsofinterest in info['class'].lower():
            continue 
        """
...
parser.add_argument('--net', 
    default='/home/javi/catkin_ws/src/DOPE-ROS-D435/dope/scripts/train_piece_no_obj_1214/net_epoch_71.pth',
...
def _runnetwork(epoch, loader, train=True):
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        """
        for index, target_be in enumerate(target_belief):
            target_be = ToPILImage()(target_be[0])
            target_be.save('target_beliefmaps/' + str(batch_idx) + str(index) + '.jpg')
        """
        loss = None
        
        # Belief maps loss
        for index, l in enumerate(output_belief): #output, each belief map layers.
            """
            l_data = ToPILImage()(l[0])
            l_data.save('beliefmaps/'+str(batch_idx) + str(index) + '.jpg')
            """
            if loss is None:
