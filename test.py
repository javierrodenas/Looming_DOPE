...
"""
import sys
sys.path.append("/home/javi/catkin_ws/src/DOPE-ROS-D435/dope/src/dope/inference/")
...
parser.add_argument('--datatest', 
    default="/home/javi/catkin_ws/src/DOPE-ROS-D435/dope/scripts/piece/",
...
parser.add_argument('--net', 
    default='/home/javi/catkin_ws/src/DOPE-ROS-D435/dope/scripts/train_piece_no_obj_1214/net_epoch_79.pth',
...
if not opt.datatest == "": 
    ...
    testingdata = torch.utils.data.DataLoader(
        MultipleVertexJson(
            ...
            transform = transform,
            normal = normal_imgs,
            target_transform = transforms.Compose([transform,
                                   transforms.Scale(opt.imagesize//8),
                ]),
            ),
        batch_size = opt.batchsize, 
...
def ADDErrorCuboid(self, pred_pose, actual_pose, cuboid):
...
def _runnetwork(epoch, loader, train=True):
    ...
    config_detect.mask_edges = 0
    config_detect.mask_faces = 0
    config_detect.vertex = 0
    config_detect.treshold = 0
    config_detect.softmax = 1000
    config_detect.thresh_angle = 20
    config_detect.thresh_map = 0.00005
    config_detect.sigma = 0
    config_detect.thresh_points = 0.00005
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        rotations     = np.array(targets['rot_quaternions'][batch_idx])
        translations  = np.array(targets['translations'][batch_idx])
        matrix_camera = np.array(targets['matrix_camera'][batch_idx])
        cuboid        = Cuboid3d(np.array(targets['cuboid'][batch_idx]))
        filename      = targets['file_name'][batch_idx]
        pointsBelief  = targets['pointsBelief'][batch_idx].numpy()


        data = Variable(targets['img'].cpu())
        data1 = ToPILImage()(data[0])  # image corresponding to x
        data2 = ToPILImage()(data[0])
#        data1 = tensor_to_image(data)
        data1.save('image{}.png'.format(batch_idx))

        output_belief, output_affinities = net(data)
        import matplotlib.pyplot as plt
        data = ToPILImage()(data[0])
        """
        for output_beliefs in output_belief:
            data = Variable(output_beliefs.cpu())
            #data = ToPILImage()(data[0])
            #data.imshow()# image corresponding to x
            data.show()
        """
        pnp_solver = CuboidPNPSolver(filename,matrix_camera,cuboid)
        detected_objects, im_belief = ObjectDetector.detect_object_in_image(net, pnp_solver, data, config_detect,
            grid_belief_debug = True, norm_belief=True,run_sampling=False,network='dope')
        
        print("aqui", detected_objects, im_belief)
        im_belief.save('belieffff.png')

        detected_objects = ObjectDetector.find_object_poses(
                    output_belief[-1][0], output_affinities[-1][0], pnp_solver, config_detect)
                    
        ###print(detected_objects)
        draw = ImageDraw.Draw(data2)

        for object in detected_objects:
            try:
                DrawCube(object['projected_points'], which_color=0, draw=draw)
                print("DOooooNEo")
            except:
                print("error")
        
        data2.show()
        data2.save("cube_detected{}.png".format(batch_idx))
