...
import colorsys
import sys
sys.path.append('/media/javi/dope/nodes/')
import math
import warnings
warnings.filterwarnings("ignore")
os.environ["CUDA_VISIBLE_DEVICES"]="0"
import sys
sys.path.append('/media/javi/dope/src/dope/inference/')
...
def main(path):
    ...
    for t in LodedTensor:
        ...
        imageToShow = cv2.resize(cv_imageBelief, dsize=(400, 400))
        """
        ...
        cv2.imwrite('{}.png'.format(relative_path), imageToShow)
...
def loadjson(path, objectsofinterest, img):
    ...
    for i_line in range(len(data['objects'])):
        ...
        #print("EeEEEEE", info['class'])
        #if not objectsofinterest is None:
        #print("EeEEEEE", info['class'])
...
class MultipleVertexJson(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img1 = transforms.ToPILImage()(img.squeeze_(0))

        #img1.save(name + '_2' + '.png')
...
def _runnetwork(epoch, loader, train=True):
    ...
    if opt.experiment_name is not None:
        wandb.init(project="LoomgniFactory", name=opt.experiment_name,
                   resume=True if opt.wandb_resume_id is not None else None,
                   id=opt.wandb_resume_id)
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        #torch.save(data, 'img_{}.pt'.format(epoch))
        ...
        for l in output_belief: #output, each belief map layers.
            if loss is None:
                ...
                train_metrics.update([(f"{type_metrics}_belief_loss", loss.item())])
            else:
                loss_tmp = ((l - target_belief) * (l-target_belief)).mean()
                train_metrics.update([(f"{type_metrics}_belief_loss", loss_tmp.item())])
                loss += loss_tmp

        # Affinities loss
        for l in output_affinities: #output, each belief map layers. 
            loss_tmp = ((l - target_affinity) * (l-target_affinity)).mean()
            train_metrics.update([(f"{type_metrics}_affinity_loss", loss_tmp.item())])
            loss += loss_tmp
        ...
        train_metrics.update([(f"{type_metrics}_loss", loss.item())])
        train_metrics.update([(f"learning_rate", opt.lr)])
...
if __name__ == "__main__":
    ...
    path1 = '/home/javi/catkin_ws/src/dope/scripts/target_belief_1.pt'
    path2 = '/home/javi/catkin_ws/src/dope/scripts/img_1.pt'
    path3 = '/home/javi/catkin_ws/src/dope/scripts/target_affinity_1.pt'
    main(path1)
    main(path2)
    main(path3)
